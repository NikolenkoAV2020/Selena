    ...
    )
{
    ...
    if (bytes_read < 16) {
        if (bytes_read <= 0) {
            //LOG(0,  "%s: failed to read header\n", capfile->filename);
            //perror(capfile->filename);
        } else if (bytes_read == 0)
            ; /* normal end-of-file */
        else
            LOG(0,  "%s: premature end-of-file\n", capfile->filename);
        return 0;
    }
    ...
    *r_original_length = PCAP32(byte_order, header+12);
    ...
    while (is_corrupt) {
        ...
        if (position == -1) {
            LOG(0,  "%s: could not resolve file corruption (ftell) frame #%" PRId64 "\n",
        ...
        LOG(0,  "%s(%" PRIu64 "): corruption found at 0x%08" PRIx64 " (%" PRId64 ")\n",
            capfile->filename,
        ...
        for (i=0; i<bytes_read; i++) {
            ...
            if (fseek_x(capfile->fp, position, SEEK_SET) != 0) {
                LOG(0,  "%s: could not resolve file corruption (seek forward)\n", capfile->filename);
                perror(capfile->filename);
                fseek_x(capfile->fp, 0, SEEK_END);
                return 0;
            }
            ...
            position = ftell_x(capfile->fp);
            LOG(0,  "%s(%" PRId64 "): good packet found at 0x%08" PRIx64 " (%" PRId64 ")\n",
                capfile->filename,
                capfile->frame_number,
                position,
                position
    ...
    if (bytes_read < *r_captured_length) {
        if (bytes_read <= 0) {
            LOG(0,  "%s: could not read packet data, frame #%" PRId64 "\n",
                capfile->filename, capfile->frame_number);
            perror(capfile->filename);
        } else
            LOG(0,  "%s: premature end of file\n", capfile->filename);
...
struct PcapFile *pcapfile_openread(const char *capfilename)
{
    ...
    if (fp == NULL) {
        LOG(0,  "%s: could not open capture file\n", capfilename);
        perror(capfilename);
        return 0;
    }
    ...
    if (bytes_read < 24) {
        if (bytes_read <= 0) {
            LOG(0,  "%s: could not read PCAP header\n", capfilename);
            perror(capfilename);
        } else if (bytes_read == 0)
            LOG(0,  "%s: file empty\n", capfilename);
        else
            LOG(0,  "%s: file too short\n", capfilename);
        fclose(fp);
        return 0;
    }
    ...
    default:
        LOG(0,  "%s: unknown byte-order in cap file\n", capfilename);
    ...
    case 119:   /* Prism II headers (also used for things like Atheros madwifi) */
        break;
    default:
        LOG(0,  "%s: unknown cap file linktype = %d (expected Ethernet or wifi)\n", capfilename, linktype);
        fclose(fp);
        return 0;
        break;
    }
    ...
    /* Read the first frame's timestamp */
    {
        ...
        loc = ftell(fp);
        if (loc == -1) {
            LOG(0,  "%s: ftell failed (file system error? seen with VMware HGFS bug)\n", capfilename);
            perror(capfilename);
            fclose(fp);
            return 0;
        }
        ...
        if (fseek(fp, loc, SEEK_SET) != 0) {
            LOG(0,  "%s: fseek failed (file system error?)\n", capfilename);
            perror(capfilename);
            fclose(fp);
            return 0;
        }
...
struct PcapFile *pcapfile_openwrite(const char *capfilename, unsigned linktype)
{
    ...
    if (!fp) {
        LOG(0,  "Could not open capture file\n");
        perror(capfilename);
        return 0;
    }
    ...
    if (fwrite(buf, 1, 24, fp) != 24) {
        LOG(0,  "Could not write capture file header\n");
        perror(capfilename);
        fclose(fp);
        return 0;
    }
...
struct PcapFile *pcapfile_openappend(const char *capfilename, unsigned linktype)
{
    ...
    if (!fp) {
        LOG(0,  "Could not open capture file to append frame\n");
        perror(capfilename);
    ...
    if (fread(buf, 1, 24, fp) != 24) {
        LOG(0,  "Error reading capture file header\n");
        perror(capfilename);
        fclose(fp);
        return pcapfile_openappend(capfilename, linktype);
    }
    ...
    if (fseek(fp, 0, SEEK_END) != 0) {
        LOG(0,  "Could not seek to end of capture file\n");
        perror(capfilename);
        fclose(fp);
        return 0;
    }
    ...
    if (linktype != file_linktype) {
        ...
        if (strstr(capfilename, linkspec) || strlen(capfilename) + linkspec_length + 1 > sizeof(newname)) {
            ...
            LOG(0,  "Giving up on appending %d-type frames onto a %d-type file\n", linktype, file_linktype);
            return 0;
        }
    ...
    unsigned time_usec)
{
    ...
    if (fwrite(header, 1, 16, capfile->fp) != 16) {
        LOG(0,  "%s: could write packet header, frame #%" PRId64 "\n",
            capfile->filename, capfile->frame_number);
        perror(capfile->filename);
        fclose(capfile->fp);
        capfile->fp = NULL;
    ...
    if (fwrite(buffer, 1, buffer_size, capfile->fp) != buffer_size) {
        LOG(0,  "%s: could write packet contents, frame #%" PRId64 "\n",
            capfile->filename, capfile->frame_number);
        perror(capfile->filename);
        fclose(capfile->fp);
        capfile->fp = NULL;

...
ASN1_push(struct CertDecode *x, unsigned next_state, uint64_t remaining)
{
    ...
    if ((remaining >> 16) != 0) {
        LOG(0,  "ASN.1 length field too big\n");
        x->state = 0xFFFFFFFF;
        return;
    }
    ...
    if (x->stack.depth >= STACK_DEPTH) {
        LOG(0,  "ASN.1 recursion too deep\n");
        x->state = 0xFFFFFFFF;
        return;
    }
...
#ifdef __cplusplus
	// Prefix increment.
	X509state& operator++(X509state& s) 
	{
		s = static_cast<X509state>(static_cast<int>(s) + 1);
		return s;
	}
	// Postfix increment.
	X509state operator++(X509state& s, int) 
	{
		X509state o = s;
		s = static_cast<X509state>(static_cast<int>(s) + 1);
		return o;
	}
	X509state operator+=(X509state& s, int n)
	{
		X509state o = s;
		s = static_cast<X509state>(static_cast<int>(s) + n);
		return o;
	}
#endif
...
{
    unsigned i;
    enum X509state state = (X509state)x->state;

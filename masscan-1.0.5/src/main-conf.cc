...
parseTime(const char *value)
{
    ...
    case 'w':
        ...
        break;
    default:
        LOG(0,  "--rotate-offset: unknown character\n");
        exit(1);
    }
    if (num >= 24*60*60) {
        LOG(0,  "--rotate-offset: value is greater than 1 day\n");
        exit(1);
    }
    if (is_negative)
        num = 24*60*60 - num;
...
parseSize(const char *value)
{
    ...
    case 'e': /* exabyte, now that's just silly */
        ...
        break;
    default:
        LOG(0,  "--rotate-size: unknown character\n");
        exit(1);
    }
    return num;
}
                      ...
                      const char *name, const char *value)
{
    ...
    if (EQUALS("conf", name) || EQUALS("config", name)) {
        ...
        if (masscan->nic[index].ifname[0]) {
            LOG(0,  "CONF: overwriting \"adapter=%s\"\n", masscan->nic[index].ifname);
        }
...
hint   addresses look like \"19.168.1.23\"\n", name, value);
            exit(1);
        }
...
hint   addresses look like \"19.168.1.23\"\n", name, value);
        ...
        for (i=0; value[i] && value[i] != '.'; i++) {
            char c = value[i];
            if (c < '0' || '9' < c) {
                LOG(0,  "CONF: non-digit in rate spec: %s=%s\n", name, value);
                return;
            }
            rate = rate * 10.0 + (c - '0');
        }
        ...
        if (EQUALS("ssl", value))
            ...
            LOG(0,  "FAIL: %s: unknown hello type\n", value);
            exit(1);
        }
        ...
        if (EQUALS("cert", value))
            ...
            LOG(0,  "FAIL: %s: unknown capture type\n", value);
            exit(1);
        }
        ...
        for (;;) {
            ...
            if (range.end < range.begin) {
                LOG(0,  "ERROR: bad IP address/range: %s\n", ranges);
                break;
            }
        ...
        if (x >= 1514 - 14 - 40) {
            LOG(0,  "error: %s=<n>: expected number less than 1500\n", name);
        } else {
        ...
        LOG(0,  "nmap(%s): unsupported: DNS lookups too synchronous\n", name);
        exit(1);
        ...
        if (count2 - count1)
        LOG(0,  "%s: excluding %u ranges from file\n",
                value, count2 - count1);
        ...
        LOG(0,  "nmap(%s): unsupported: this is an asynchronous tool, so no timeouts\n", name);
        exit(1);
        ...
        LOG(0,  "nmap(%s): unsupported: maybe soon\n", name);
        exit(1);
        ...
        LOG(0,  "nmap(%s): unsupported: we randomize all the groups!\n", name);
        exit(1);
    } else if (EQUALS("min-parallelism", name) || EQUALS("max-parallelism", name)) {
        LOG(0,  "nmap(%s): unsupported: we all the parallel!\n", name);
        exit(1);
    } else if (EQUALS("min-rtt-timeout", name) || EQUALS("max-rtt-timeout", name) || EQUALS("initial-rtt-timeout", name)) {
        LOG(0,  "nmap(%s): unsupported: we are asychronous, so no timeouts, no RTT tracking!\n", name);
        exit(1);
    } else if (EQUALS("min-rate", name)) {
        LOG(0,  "nmap(%s): unsupported, we go as fast as --max-rate allows\n", name);
        /* no exit here, since it's just info */
    } else if (EQUALS("mtu", name)) {
        LOG(0,  "nmap(%s): fragmentation not yet supported\n", name);
        exit(1);
        ...
        LOG(0,  "nmap(%s): OS scanning unsupported\n", name);
        exit(1);
    } else if (EQUALS("osscan-guess", name)) {
        LOG(0,  "nmap(%s): OS scanning unsupported\n", name);
        exit(1);
    } else if (EQUALS("output-format", name)) {
        enum OutputFormat x = (OutputFormat)0;
...
hint: 'binary', 'xml', 'grepable', ...\n", value);
		...
		if (strlen(masscan->output.resultdir)) {
			snprintf(masscan->output.filename,
				sizeof(masscan->output.filename), "%s", value);
		} else {
			strcpy_s(masscan->output.filename,
				sizeof(masscan->output.filename), value);
		}
		...
		if (strlen(masscan->output.resultdir)) {
			snprintf(masscan->output.filename,
				...
				"%s/<redis>", masscan->output.resultdir);
		}
		else {
			strcpy_s(masscan->output.filename,
				sizeof(masscan->output.filename),
        ...
        LOG(0,  "nmap(%s): this is our default option\n", name);
        ...
        unsigned x = strtoul(value, 0, 0);
        if (x >= 1000) {
            LOG(0,  "error: retries=<n>: expected number less than 1000\n");
        } else {
            masscan->retries = x;
        }
        ...
        if (!script_lookup(value)) {
            LOG(0,  "FAIL: script '%s' does not exist\n\
  ...
  hint: use '--script list' to list available scripts\n", value);
        ...
        if (masscan->script.name != NULL) {
            if (strcmp(masscan->script.name, value) == 0)
                ...
                LOG(0,  "FAIL: only one script supported at a time\n\
  hint: '%s' is existing script, '%s' is new script\n", masscan->script.name, value);
                exit(1);
            }
        }
        ...
        LOG(0,  "nmap(%s): unsupported: we do timing VASTLY differently!\n", name);
        exit(1);
    } else if (EQUALS("scanflags", name)) {
        LOG(0,  "nmap(%s): TCP scan flags not yet supported\n", name);
        exit(1);
        ...
        LOG(0,  "nmap(%s): unnecessary, we always do --send-eth\n", name);
    } else if (EQUALS("send-ip", name)) {
        LOG(0,  "nmap(%s): unsupported, we only do --send-eth\n", name);
        exit(1);
...
hint   it goes like 1/4 2/4 3/4 4/4\n");
            exit(1);
        }
        if (one > of) {
...
hint   it goes like 1/4 2/4 3/4 4/4\n");
        ...
        LOG(0,  "nmap(%s): DNS lookups will never be supported by this code\n", name);
        exit(1);
        ...
        unsigned x = strtoul(value, 0, 0);
        if (x >= 256) {
            LOG(0,  "error: %s=<n>: expected number less than 256\n", name);
        } else {
            masscan->nmap.ttl = x;
        }
        ...
        LOG(0,  "CONF: unknown config option: %s=%s\n", name, value);
    }
}
...
masscan_command_line(struct Masscan *masscan, int argc, char *argv[])
{
    ...
    for (i=1; i<argc; i++) {
        ...
        if (argv[i][0] == '-' && argv[i][1] == '-') {
            if (strcmp(argv[i], "--help") == 0) {
                ...
                if (i >= argc) {
                    LOG(0,  "%.*s: empty parameter\n", name_length, name);
                    break;
                }

                if (name_length > sizeof(name2) - 1) {
                    LOG(0,  "%.*s: name too long\n", name_length, name);
        ...
        if (argv[i][0] == '-') {
            ...
            switch (argv[i][1]) {
            case '6':
                LOG(0,  "nmap(%s): unsupported: maybe one day\n", argv[i]);
                exit(1);
            case 'A':
                LOG(0,  "nmap(%s): unsupported: this tool only does SYN scan\n", argv[i]);
                exit(1);
            case 'b':
                LOG(0,  "nmap(%s): FTP bounce scans will never be supported\n", argv[i]);
                exit(1);
            case 'c':
                if (argv[i][2])
                    arg = argv[i]+2;
            ...
            case 'f':
                ...
                exit(1);
            case 'F':
                LOG(0,  "nmap(%s): unsupported, no slow/fast mode\n", argv[i]);
                exit(1);
            case 'g':
                if (argv[i][2])
                    arg = argv[i]+2;
            ...
            case 'i':
                if (argv[i][3] == '\0' && !isdigit(argv[i][2]&0xFF)) {
                    ...
                    case 'R':
                        ...
                        break;
                    default:
                        LOG(0,  "nmap(%s): unsupported option\n", argv[i]);
                        exit(1);
                    }
            ...
            case 'o': /* nmap output format */
                ...
                case 'A':
                    ...
                    LOG(0,  "nmap(%s): unsupported output format\n", argv[i]);
                    exit(1);
                    break;
                case 'B':
                ...
                case 'U':
                    ...
                    break;
                default:
                    LOG(0,  "nmap(%s): unknown output format\n", argv[i]);
                    exit(1);
                }

                ++i;
                if (i >= argc || (argv[i][0] == '-' && argv[i][1] != '\0')) {
                    LOG(0,  "missing output filename\n");
                    exit(1);
                }
                ...
                break;
            case 'O':
                LOG(0,  "nmap(%s): unsupported, OS detection is too complex\n", argv[i]);
                exit(1);
                break;
            case 'p':
                if (argv[i][2])
            ...
            case 'P':
                ...
                case 'n':
                    ...
                    break;
                default:
                    LOG(0,  "nmap(%s): unsupported option, maybe in future\n", argv[i]);
                    exit(1);
                }
                break;
            case 'r':
                /* This looks like an nmap option*/
                LOG(0,  "nmap(%s): wat? randomization is our raison d'etre!! rethink prease\n", argv[i]);
                exit(1);
                break;
            case 'R':
                /* This looks like an nmap option*/
                LOG(0,  "nmap(%s): unsupported. This code will never do DNS lookups.\n", argv[i]);
                exit(1);
                break;
            case 's': /* NMAP: scan type */
                if (argv[i][3] == '\0' && !isdigit(argv[i][2]&0xFF)) {
                    ...
                    switch (argv[i][j]) {
                    case 'A':
                        LOG(0,  "nmap(%s): ACK scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'C':
                        LOG(0,  "nmap(%s): unsupported\n", argv[i]);
                        exit(1);
                    case 'F':
                        LOG(0,  "nmap(%s): FIN scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'I':
                        LOG(0,  "nmap(%s): Zombie scans will never be supported\n", argv[i]);
                        exit(1);
                    case 'L': /* List Scan - simply list targets to scan */
                        masscan->op = Operation_ListScan;
                        break;
                    case 'M':
                        LOG(0,  "nmap(%s): Maimon scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'n': /* Ping Scan - disable port scan */
                        LOG(0,  "nmap(%s): ping-sweeps not yet supported\n", argv[i]);
                        exit(1);
                    case 'N':
                        LOG(0,  "nmap(%s): NULL scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'O':
                        LOG(0,  "nmap(%s): IP proto scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'S': /* TCP SYN scan - THIS IS WHAT WE DO! */
                        masscan->scan_type.tcp = 1;
                        break;
                    case 'T': /* TCP connect scan */
                        LOG(0,  "nmap(%s): connect() is too synchronous for cool kids\n", argv[i]);
                        LOG(0,  "WARNING: doing SYN scan anyway\n");
                        break;
                    case 'U': /* UDP scan */
                        masscan->scan_type.udp = 1;
                        break;
                    case 'V':
                        LOG(0,  "nmap(%s): unlikely this will be supported\n", argv[i]);
                        exit(1);
                    case 'W':
                        LOG(0,  "nmap(%s): Windows scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'X':
                        LOG(0,  "nmap(%s): Xmas scan not yet supported\n", argv[i]);
                        exit(1);
                    case 'Y':
                        break;
                    case 'Z':
            ...
            case 'W':
                ...
                return;
            case 'T':
                LOG(0,  "nmap(%s): unsupported, we do timing WAY different than nmap\n", argv[i]);
                exit(1);
                return;
            default:
 ...
 [hint] ...or, to list nmap-compatible options, try \"--nmap\"\n", argv[i]);
        ...
        if (!isdigit(argv[i][0])) {
            LOG(0,  "FAIL: unknown command-line parameter \"%s\"\n\
 [hint] did you want \"--%s\"?\n", argv[i], argv[i]);
            exit(1);
        }
...
mainconf_selftest()
{
    ...
    /* */
    {
        ...
        if (masscan_conf_contains("--nothing", argc, (char**)&argv[0]))
            return 1;

        if (!masscan_conf_contains("--readscan", argc, (char**)&argv[0]))
            return 1;
    }

...
#include <stdarg.h>
	...
	#include <WinSock.h>
	#if defined(_MSC_VER)
		#pragma message("		Connecting ... Ws2_32.lib")
...
struct ThreadPair {
	 ...
	 * unsafe */
	const struct Masscan *masscan = 0;

	// For receive thread function
	struct MasScanReport  MSR;
	struct Output         *out = 0;
	struct PcapFile       *pcapfile = 0;
	struct DedupTable     *dedup = 0;
...
receive_thread(void *v)
{
	struct ThreadPair      *parms = (struct ThreadPair*)v;
#ifdef _LIB
	struct MasScanReport     *MSR = &parms->MSR;
#endif
	const struct Masscan *masscan = parms->masscan;
	struct Adapter       *adapter = parms->adapter;
	...
	if (masscan->is_banners) {
		...
		parms->tcpcon = tcpcon_create_table(
		...
		tcpcon_set_banner_flags(parms->tcpcon,
		...
		if (masscan->http_user_agent_length)
			tcpcon_set_parameter(parms->tcpcon,
				"http-user-agent",
	...
	while (!is_rx_done) {
		...
		if (rawsock_recv_packet(
			adapter, &length, 
			&secs, &usecs, 
			&px)) 
		{
			tcpcon_timeouts(parms->tcpcon, (unsigned)time(0), 0);
			continue;
		}
		 ...
		 */
		tcpcon_timeouts(parms->tcpcon, secs, usecs);
		...
		if (!is_my_ip(&parms->src, ip_me)) continue;
		...
		case FOUND_ARP:
			...
			case 2: /* response */
				...
				/* Ignore duplicates */
				if (dedup_is_duplicate(parms->dedup, ip_them, 0, ip_me, 0))
					continue;

				/* ...everything good, so now report this response */
				handle_arp(parms->out, secs, px, length, &parsed);
				break;
			}
			continue;
		case FOUND_UDP:
		case FOUND_DNS:
			...
			handle_udp(parms->out, secs, px, length, &parsed, entropy);
			continue;
		case FOUND_ICMP:
			handle_icmp(parms->out, secs, px, length, &parsed, entropy);
			continue;
		case FOUND_SCTP:
			handle_sctp(parms->out, secs, px, length, cookie, &parsed, entropy);
			break;
		case FOUND_TCP:
			/* fall down to below */
			break;
		...
		if (parms->tcpcon) {
			...
			tcb = tcpcon_lookup_tcb(parms->tcpcon,	ip_me,
						ip_them, port_me, port_them);
			...
			if (TCP_IS_SYNACK(px, parsed.transport_offset)) {
				...
				if (tcb == NULL) {
					tcb = tcpcon_create_tcb(parms->tcpcon,
								ip_me, ip_them,
								port_me, port_them,
		...
		if (TCP_IS_SYNACK(px, parsed.transport_offset) ||
			...
			if (dedup_is_duplicate(parms->dedup, ip_them, port_them, ip_me, port_me))
				continue;
...
#			endif 
			...
			// Intermediate file with host ip-addresses for nmap
			//if (parms->out->fpnminput) {
			//	unsigned ip = ip_them;
			//	short ip1 = (ip >> 24) & 0xFF;
			//	short ip2 = (ip >> 16) & 0xFF;
			//	short ip3 = (ip >> 8) & 0xFF;
			//	short ip4 = (ip >> 0) & 0xFF;
			//	char  ipStr[32];
			//	sprintf_s(ipStr, 32, "%d.%d.%d.%d", ip1, ip2, ip3, ip4);
			//	fprintf(parms->out->fpnminput, "%s\n", ipStr);
			//}
			...
			if (!parms->tcpcon && !masscan->is_noreset)
				tcp_send_RST(
					&parms->tmplset->pkts[Proto_TCP],
					parms->packet_buffers,
					parms->transmit_queue,
	...
	output_destroy(parms->out);				parms->out = 0;
	dedup_destroy(parms->dedup);			parms->dedup = 0;
	tcpcon_destroy_table(parms->tcpcon);	parms->tcpcon = 0;
	pcapfile_close(parms->pcapfile);		parms->pcapfile = 0;

	for (;;) {
		void *p;
		if (!rte_ring_sc_dequeue(parms->packet_buffers, (void**)&p)) free(p);
		else break;
	}
...
main_scan(struct Masscan *masscan)
{
 ...
 [hint] use \"--exclude 255.255.255.255\" as a simple confirmation\n");
	...
	for (index = 0; index < masscan->nic_count; index++) {
		...
		memset(&parms->MSR, 0, sizeof(parms->MSR));
		...
		parms->out = output_create(masscan, index);
		...
		err = masscan_initialize_adapter(
			...
			parms->router_mac);
		if (err) {
			LOG(0, "FAIL: failed to initialize adapter\n");
			exit(1);
		}
		parms->adapter = masscan->nic[index].adapter;
		if (!masscan->nic[index].src.ip.range) {
...
#		define BUFFER_COUNT 16384
	 ...
	 */
	{
		...
		"\nStarting masscan %s (http://bit.ly/14GZzcT) at %s\n", MASSCAN_VERSION, buffer);
		LOG(0, message);

		masscan->MSR.countHost = (unsigned long)count_ips;
		masscan->MSR.countPort = (unsigned long)count_ports;
		if (masscan->ports.list->begin == Templ_ICMP_echo && 
			masscan->ports.list->end   == Templ_ICMP_echo && count_ports == 1)
		{ 
			// ICMP only 
			LOG(0, " -- forced options: -sn -n --randomize-hosts -v --send-eth\n");
			snprintf(message, sizeof(message),
				"Initiating ICMP Echo Scan. Scanning %lu hosts", count_ips);
			LOG(0, "%s\n", message);
#ifdef _LIB
			MessageToClient(std::string("Masscan: ") + message, 2);
#endif
		} else {
			// This could actually also be a UDP only or mixed UDP/TCP/ICMP scan 
			LOG(0, " -- forced options: -sS -Pn -n --randomize-hosts -v --send-eth\n");
			snprintf(message, sizeof(message),
				"Initiating SYN Stealth Scan. Scanning %lu hosts [%lu port%s/host]", 
				count_ips, count_ports, ((count_ports == 1) ? "" : "s"));
			LOG(0, "%s\n", message);
#ifdef _LIB
			MessageToClient(std::string("Masscan: ") + message, 2);
#endif
	...
	while (!is_tx_done && masscan->output.is_status_updates) {
		...
		for (i = 0; i < masscan->nic_count; i++) {
			...
			if (parms->total_tcbs   ) total_tcbs    += parms->total_tcbs;
			if (parms->total_synacks) total_synacks += parms->total_synacks;
			if (parms->total_syns   ) total_syns    += *parms->total_syns;
		}
		 ...
		 */
		masscan->MSR.countPortOpen = (unsigned long)total_synacks;
		if (masscan->output.is_status_updates)
			status_print(&status, min_index, range, rate,
				total_tcbs, total_synacks, total_syns, 0);
	...
	for (;;) {
		...
		if (time(0) - now >= masscan->wait)	is_rx_done = 1;
		...
		if (masscan->output.is_status_updates) {
			...
			if (transmit_count < masscan->nic_count) continue;
			is_tx_done = 1;
			is_rx_done = 1;
			if (receive_count < masscan->nic_count)	continue;
		}
		else {
			/* [AFL-fuzz]
	...
	masscan->MSR.countHostOpen = 0;
	masscan->MSR.countPortOpen = 0;
	for (unsigned int i = 0; i < masscan->nic_count; i++) {
		struct ThreadPair *parms = &parms_array[i];

		masscan->MSR.countHostOpen += parms->MSR.countHostOpen;
		masscan->MSR.countPortOpen += parms->MSR.countPortOpen;
...
#ifdef _LIB
//
// A few code for redirect all masscan output to log file
// all printf & fprintf functions in source code redefined
// accordingly as newPrintf & newFprintf
//
struct Masscan* globalMasscan = NULL;
char   lastError[512] = "";
...
int newPrintf(const char *fmt, ...)
{
	...
	if (globalMasscan && globalMasscan->output.fileLog) {
		r = vfprintf(globalMasscan->output.fileLog, fmt, ap);
	} else {
		r = vprintf(fmt, ap);
	}
...
int newFprintf(FILE* f, const char *fmt, ...)
{
	if (!fmt || !*fmt)	return 0;
	int r = 0;
...
void newPerror(const char *msg)
{
	newFprintf(stderr, "%s. Error: %d.\n", msg, errno);
}

// Redirect exit function
void newExit(int errCode)
{
	if (errCode) {
		if (!lastError[0]) {
			snprintf(lastError, sizeof(lastError), 
			"Unsuccessful attempt to scan using masscan API");
	}	}
	// throw an exception for the correct program termination 
	throw(errCode);
}
	...
	struct MasScanReport* MSR)
{
	...
	// if masscan was callin like library whith command, than
	// select command substrings, allocating memory for them.
	DecodeCommandString(Command, &countString, &strings);
	// This memory (strings) further must be freed
	try {
		r = newMain(countString, strings, outDir, uniqueName);
	} catch (int errCode) {
		r = errCode;
	}
	// if masscan was callin like library whith command,
	// then delete the memory allocated for the command substrings
	DecodeCommandStringFree(&countString, &strings);

	if (globalMasscan) {
		memcpy(MSR, &globalMasscan->MSR, sizeof(struct MasScanReport));
		fclose(globalMasscan->output.fileLog);
		globalMasscan->output.fileLog = NULL;
	}
	lastError[0] = '\0';

	// Free all allocated memory
	if (globalMasscan) {
		rangelist_remove_all(&globalMasscan->ports);
		rangelist_remove_all(&globalMasscan->targets);
		rangelist_remove_all(&globalMasscan->banner_types);
		rangelist_remove_all(&globalMasscan->exclude_port);
		rangelist_remove_all(&globalMasscan->exclude_ip);
	}

	return r;
}

void GetlastStatus(char s[256])
{
	s[0] = '\0';
	memcpy(s, lastStatus, sizeof(lastStatus));
}
...
{
#ifndef _LIB
	int countString = argc;
	char**  strings = argv;
#endif // #ifndef _LIB

	// Set the initial values to global variables for the possibility 
	// of repeated start of this procedure
	is_tx_done = 0;
	is_rx_done = 0;
	...
	{
		int is_backtrace = 1;
		for (i = 1; i < (unsigned)countString; i++) {
			if (strcmp(strings[i], "--nobacktrace") == 0)
				is_backtrace = 0;
		}
		if (is_backtrace)
			pixie_backtrace_init(strings[0]);
	}
...
#ifdef _LIB
	snprintf(masscan->output.resultdir,
		sizeof(masscan->output.resultdir), "%s", outDir);
	snprintf(masscan->output.uniqueName,
		sizeof(masscan->output.uniqueName),	"%s", uniqueName);
	snprintf(masscan->output.uniqueLogName,
		sizeof(masscan->output.uniqueLogName), "%s/%s.jobl", outDir, uniqueName);
	fopen_s(&masscan->output.fileLog, masscan->output.uniqueLogName, "w");
	globalMasscan = masscan;
#else
	masscan->output.resultdir[0] = '\0';
	masscan->output.uniqueLogName[0] = '\0';
	masscan->output.fileLog = NULL;
#endif // #ifdef _LIB

	/*
	 * Pre-parse the command-line
	 */
	if (masscan_conf_contains("--readscan", countString, strings)) {
	 ...
	 */
	masscan_command_line(masscan, countString, strings);
   ...
   ...all ranges overlapped something in an excludefile range\n");
			exit(1);
		}

		if (range2 != range && masscan->resume.index) {
			LOG(0, "FAIL: Attempted to add additional 'exclude' ranges after scan start.\n\
   ...This messes things up the scan randomization, so you have to restart scan\n");
			exit(1);
		}
	}
	...
	case Operation_ReadScan:
	{
		...
		// find first file
		for (start = 1; start < (unsigned)countString; start++) {
			if (memcmp(strings[start], "--readscan", 10) == 0) {
				start++;
				break;
		}	}

		// find last file
		for (stop = start + 1; stop < (unsigned)countString && strings[stop][0] != '-'; stop++)
			;

		// read the binary files, and output them again depending upon
		// the output parameters
		read_binary_scanfile(masscan, start, stop, strings);
		 ...
		 */
	{
		...
		if (x) {
			...
			LOG(0, "regression test: failed :( \n");
			r = 1;
		} else {
			LOG(0, "regression test: success!\n");
		}
	}
	break;
	}
	...
	rangelist_remove_all(&masscan->ports);
	rangelist_remove_all(&masscan->targets);
	rangelist_remove_all(&masscan->banner_types);
	rangelist_remove_all(&masscan->exclude_port);
	rangelist_remove_all(&masscan->exclude_ip);

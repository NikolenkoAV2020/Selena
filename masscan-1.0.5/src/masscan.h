...
#include <SelenaMessage.h>

#ifdef _LIB
	int  newPrintf (const char *fmt, ...);
	int  newFprintf(FILE*, const char *fmt, ...);
	void newExit   (int);
	void newPerror (const char*);
#	define	printf  newPrintf
#	define	fprintf newFprintf
#	define  exit    newExit
#	define  perror  newPerror
#else
...
struct Masscan
{
    ...
    struct OutputStuff {
		...
		// The directory to which we store result of scaning
		char resultdir[256];
		char uniqueName[256];

		// log-file for using masscan as static library
		char uniqueLogName[256];
		FILE* fileLog;

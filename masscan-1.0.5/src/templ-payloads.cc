...
struct Payload2 hard_coded_payloads[] = {
    ...
    {53, 65536, 0x1f, 0, dns_set_cookie,
        /* 00 */(char*)("\x50\xb6"  /* transaction id */
        /* 02 */"\x01\x20"  /* quer y*/
        /* 04 */"\x00\x01"  /* query = 1 */
    ...
    {123, 65536, 48, 0, ntp_set_cookie,
        (char*)("\x17\x00\x03\x2a\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00")
    },
    {137, 65536, 50, 0, dns_set_cookie,
        (char*)("\xab\x12" /* transaction id */
        "\x00\x00" /* query */
    ...
    {5060, 65536, 0xFFFFFFFF, 0, 0,
        (char*)("OPTIONS sip:carol@chicago.com SIP/2.0\r\n"
    ...
    {11211, 65536, 15, 0, memcached_udp_set_cookie,
        (char*)("\x00\x00\x00\x00\x00\x01\x00\x00stats\r\n")
    },
...
{
    unsigned i;
    struct Payload **list2 = NULL;
    unsigned count2 = 0;

    /* Create a new list */
	if (payloads->max >= SIZE_MAX / sizeof(list2[0])) {
		LOG(0, "Integer overflow\n");
		exit(1);
	} else {
                   ...
                   struct NmapPayloads *payloads)
{
    ...
    if (pcap == NULL) {
        LOG(0,  "payloads: can't read from file '%s'\n", filename);
        return;
    }
                   ...
                   struct NmapPayloads *payloads)
{
    ...
    for (;;) {
        ...
        if (memcmp(line, "udp", 3) != 0) {
            LOG(0,  "%s:%u: syntax error, expected \"udp\".\n",
        ...
        if (is_error) {
            LOG(0,  "%s:%u: syntax error, expected ports\n",
                filename, line_number);
            goto end;
        }
        ...
        if (memcmp(line, "source", 6) == 0) {
            ...
            trim(line, sizeof(line));
            if (!isdigit(line[0])) {
                LOG(0,  "%s:%u: expected source port\n",
                        filename, line_number);
                goto end;
            }

    ...
    )
{
    ...
    if (adapter_ip == 0) {
        ...
        LOG(0,  " [hint] if it has no IP address, manually set with "
    ...
    if (masscan->nic[index].my_mac_count == 0) {
        ...
        if (memcmp(adapter_mac, "\0\0\0\0\0\0", 6) == 0) {
            LOG(0,  "FAIL: failed to detect MAC address of interface:"
                    " \"%s\"\n", ifname);
            LOG(0,  " [hint] try something like "

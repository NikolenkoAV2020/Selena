...
smack_create(const char *name, unsigned nocase)
{
    ...
    if (!smack) {
		LOG(0, "Smack: out of memory error\n");
        exit(1);
    }
...
smack_stage0_compile_prefixes(struct SMACK *smack)
{
     ...
     * to the table.
     */
    for (a=0; a<(unsigned)smack->m_pattern_count; a++)
...
smack_compile(struct SMACK *smack)
{
     ...
     */
    smack->m_state_max = 1;
    for (i=0; i<(unsigned)smack->m_pattern_count; i++) {
...
smack_selftest(void)
{
...

    {
        ...
        i = 0;
#define TEST(pat, offset, str) if (pat != id || offset != i) return 1 + LOG(0,  "smack: fail %s\n", str)

...
static int is_pcap_file = 0;
	...
	#if defined(_MSC_VER)
		#pragma message("		Connecting ... IPHLPAPI.lib")
...
#endif // defined(WIN32) && !defined(STATICPCAP)
...
rawsock_list_adapters(void)
{
    ...
    if (PCAP.findalldevs(&alldevs, errbuf) != -1) {
        ...
        if (alldevs == NULL) {
            LOG(0,  "ERR:libpcap: no adapters found, are you sure you are root?\n");
        }
        /* Print the list */
        for(d=alldevs; d; d=PCAP.dev_next(d)) {
            LOG(0,  " %d  %s \t", i++, PCAP.dev_name(d));
            if (PCAP.dev_description(d))
                LOG(0,  "(%s)\n", PCAP.dev_description(d));
            else
                LOG(0,  "(No description available)\n");
        }
        LOG(0, "\n");
...
rawsock_ignore_transmits(struct Adapter *adapter, const unsigned char *adapter_mac)
{
    ...
    if (adapter->pcap) {
        ...
        if (err) {
            PCAP.perror(adapter->pcap, (char*)("pcap_setdirection(IN)"));
        }
    }
                     ...
                     unsigned vlan_id)
{
    ...
    if (is_numeric_index(adapter_name)) {
        ...
        if (new_adapter_name == 0) {
            LOG(0,  "pcap_open_live(%s) error: bad index\n",
                    adapter_name);
            return 0;
        } else
    ...
    if(is_pfring && !is_pfring_dna(adapter_name)){ /*First ensure pfring dna adapter is available*/
        LOG(0, "No pfring adapter available. Please install pfring or run masscan without the --pfring option.\n");
        return 0;
    }
     ...
     *----------------------------------------------------------------*/
    {
        ...
        if (memcmp(adapter_name, "file:", 5) == 0) {
            ...
            adapter->pcap = (pcap_t*)PCAP.open_live(
    ...
    adapter->sendq = 0;
#if defined(WIN32)
    if (is_sendq) adapter->sendq = PCAP.sendqueue_alloc(SENDQ_SIZE);
#endif
...
{
    if (!memcmp(lhs, "\\Device\\NPF_", 12)) lhs += 12;
    if (!memcmp(rhs, "\\Device\\NPF_", 12)) rhs += 12;
...
rawsock_selftest_if(const char *ifname)
{
    ...
    if (ifname == NULL || ifname[0] == 0) {
        ...
        if (err) {
            LOG(0,  "get-default-if: returned err %d\n", err);
            return -1;
        }
        ifname = ifname2;
    }
    ...
    if (err) {
        LOG(0,  "get-adapter-mac: returned err=%d\n", err);
    } else {
    ...
    if (err) {
        LOG(0,  "get-default-gateway: returned err=%d\n", err);
    } else {

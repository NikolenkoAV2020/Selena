     ...
     */
    {
        ...
        if (sdl) {
            ...
            if (memcmp(ifname, sdl->sdl_data, sdl->sdl_nlen) != 0) {
                LOG(0,  "ERROR: ROUTE DOESN'T MATCH INTERFACE\n");
                LOG(0,  "YOU'LL HAVE TO SET --router-mac MANUALLY\n");
                exit(1);
            }
        }
...
read_netlink(int fd, char *bufPtr, size_t sizeof_buffer, int seqNum, int pId)
{
 ...
 do {
	  ...
	  (nlHdr->nlmsg_seq != (unsigned int)seqNum) || 
...
int rawsock_get_default_gateway(const char *ifname, unsigned *ipv4)
{
    ...
    *ipv4 = 0;
    ...
    if (fd < 0) {
        LOG(0,  "%s:%d: socket(NETLINK_ROUTE): %d\n",
    ...
    if (send(fd, nlMsg, nlMsg->nlmsg_len, 0) < 0) {
        LOG(0,  "%s:%d: send(NETLINK_ROUTE): %d\n",
            __FILE__, __LINE__, errno);
        return errno;
    }
...
int rawsock_get_default_gateway(const char *ifname, unsigned *ipv4)
{
    ...
    if (pAdapterInfo == NULL) {
        LOG(0,  "Error allocating memory needed to call GetAdaptersinfo\n");
        return EFAULT;
    }
...
again:
    ...
    if (err != NO_ERROR) {
        LOG(0,  "GetAdaptersInfo failed with error: %u\n", 
                            (unsigned)err);
        return EFAULT;
    }

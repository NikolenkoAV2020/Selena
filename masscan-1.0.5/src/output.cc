...
open_rotate(struct Output *out, const char *filename)
{
    ...
    if (fp == 0) {
        ...
        if (x != 0 || fp == NULL) {
            LOG(0,  "out: could not open file for %s\n",
...
close_rotate(struct Output *out, FILE *fp)
{
    if (!out || !fp) return;
	//if (out->fpnminput) {
	//	fflush(out->fpnminput);
	//	fclose(out->fpnminput);
	//	out->fpnminput = NULL;
	//}

    // Write the format-specific trailers, like </xml>
    if (!out->is_virgin_file) out->funcs->close(out, fp);
    memset(&out->counts, 0, sizeof(out->counts));

    // Redis Kludge
    if (out->format == Output_Redis) return;
...
output_create(const struct Masscan *masscan, unsigned thread_index)
{
    ...
    if (masscan->output.filename[0] && out->funcs != &null_output) {
		...
		// Build name op nmap input file
		//out->nmapIntermediateFileName[0] = '\0';
		//out->nmapOutputFileName[0]       = '\0';
//#		ifdef _LIB
			//snprintf(out->nmapIntermediateFileName, 256,
			//	"%s/%snminput.txt",
			//	masscan->output.resultdir,
			//	masscan->output.uniqueName);
			//snprintf(out->nmapOutputFileName, 256,
			//	"%s/%snmoutput.txt",
			//	masscan->output.resultdir,
			//	masscan->output.uniqueName);

			// open file for nmap input
			//fopen_s(&out->fpnminput, out->nmapIntermediateFileName, "w");
			//if (out->fpnminput == NULL) {
			//	perror(out->nmapIntermediateFileName);
			//	exit(1);
			//}
//#		endif
    }
    ...
    if (out->is_interactive || out->format == 0 || out->format == Output_Interactive) {
        ...
        case 0: /* ARP */
            count = fprintf(stdout, "Discovered %s port %u/%s on %u.%u.%u.%u (%02x:%02x:%02x:%02x:%02x:%02x) %s",
                        status_string((PortStatus)status),
                        port,
                ...
                const unsigned char *px, unsigned length)
{
    ...
    if (out->is_interactive || out->format == 0 || out->format == Output_Interactive) {
        ...
        count = fprintf(stdout, "Banner on port %u/%s on %u.%u.%u.%u: [%s] %s",
            ...
            (ip>> 8)&0xFF,
            (ip>> 0)&0xFF,
            masscan_app_to_string((ApplicationProtocol)proto),
     ...
     * and so on.
     */
    out->funcs->banner(out, fp, now, ip, ip_proto, port, (ApplicationProtocol)proto, ttl, px, length);

...
{
	return (int)sendto(s, (const char*)resp, 
		ntohs(resp->msglen) + sizeof(*resp), 0,
...
static void stun_req_id(struct stun_header *req)
{
	...
	char x = 20;
	do {
		uint32_t s_elm = (uint32_t)S_block[(uint8_t)randfiller];
		randfiller = (randfiller << 5) | (randfiller >> (64 - 5));
		randfiller += s_elm ^ x;
		req->id.id[x & 3] += (unsigned int)(randfiller ^ (randfiller >> 19));
	} while (--x);
...
static int stun_get_mapped(struct stun_attr *attr, void *arg)
{
	...
	if (rc) {
			...
			// for Windows only
			sa->sin_port	= (unsigned short)(addr->port ^ xor_mask);
		#elif defined(__linux__)
			// For Unix, Linux and so on...
			sa->sin_port	= (in_port_t)(addr->port ^ xor_mask);
		#else					// #if defined(__linux__)
			// anything other...
			#error Ñompilation for unknown platform.
		#endif					// #ifdef WINDOWS
...
static int StunRequest2(int sock, struct sockaddr_in *server, struct sockaddr_in *mapped) {
	...
	*/
	res = (int)recvfrom(sock, (char *)reply_buf, 
			sizeof(reply_buf) - 1, 0, (struct sockaddr *)&src, &srclen);
	if (res <= 0) return -12;
...
static int StunRequest(const char *host, uint16_t port, struct sockaddr_in *mapped, uint16_t src_port) {
	...
	// Close the socket is platform depended operation
	#if defined(WIN32) || defined(WINDOWS)
		// for Windows only
		closesocket(sock);
	#elif defined(__linux__)

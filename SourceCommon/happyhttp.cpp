...
void Connection::connect()
{
	...
	if(!addr) throw Wobbly( "Invalid network address" );
...
void Connection::putrequest( const char* method, const char* url )
{
	...
	char req[ 512 ];
	snprintf( req, sizeof(req), "%s %s HTTP/1.1", method, url );
...
{
	char buf[32];
	snprintf( buf, sizeof(buf), "%d", numericvalue );
...
/*
	std::cout << "ffffffffffffff gggggggggggggggg jjjjjjjjjjjjjjj\n";
	std::cout << "11111111111111111111gggggggggggg jjjjjjjjjjjjjjj\n";
	std::cout << "2222222222222222222222222ggggggggggg jjjjjjjjjjjjjjj\n";
	std::cout << "3333333333333333333333333333333ggg jjjjjjjjjjjjjjj\n";
	std::cout << "44444444444444444444444444444gggggggggg jjjjjjjjjjjjjjj\n";
	...
	std::cin.rdbuf(psbuf);         // assign streambuf to cout
	...
	//char sss[1024];
	//strbuf.pubsetbuf(sss, 1024);
	//std::streambuf *cintbuf = std::cin.rdbuf();
	//std::streambuf *outbuf  = std::cout.rdbuf();
	//std::streambuf *psbuf = outstr.rdbuf();
	//std::cin.rdbuf(&strbuf);
	std::cout << "\x1b[6n";
	...
	//ss = strbuf.str();
	//std::cin >> ss;
	//std::cin.rdbuf(cintbuf);
	//std::cout.rdbuf(outbuf);

	//"\x1b[1A" - курсор вверх на одну строку(параметром задаётся)
	//"\x1b[0J" - стирание от тек позиции до конца экрана
	//"\x1b[0K" - стирание от тек позиции до конца строки

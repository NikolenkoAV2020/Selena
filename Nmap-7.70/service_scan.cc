...
{
	if (global_AP) {
		delete global_AP; global_AP = NULL;
	}
}
...
int service_scan(std::vector<Target *> &Targets) {
  ...
  char message[1024];
  snprintf(message, sizeof(message), "Start Service scaning for %d targets.", (int)Targets.size());
  ...
  PutNmapGlobalMessage("\tService scaning done.");

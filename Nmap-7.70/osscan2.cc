...
static void doSeqTests(OsScanInfo *OSI, HostOsScan *HOS) 
{
	...
	// # of times in a row that hosts were unable to send probe 
	unsigned int	unableToSend = 0;  
	...
	snprintf(message, sizeof(message), 
		"Start sequence generation tests for %d Targets.", 
		(int)OSI->incompleteHosts.size());
	PutNmapGlobalMessage(message);
	...
	PutNmapGlobalMessage("\tThe sequence generation tests done.");
}
...
static void doTUITests(OsScanInfo *OSI, HostOsScan *HOS) 
{
	...
	snprintf(message, sizeof(message), 
		"Start TCP, UDP, ICMP Tests for %d targets", 
		(int)OSI->incompleteHosts.size());
	PutNmapGlobalMessage(message);
	...
	do {
		...
		do {
			...
			if (bytes < (4 * ip->ip_hl) + 4U) continue;
		...
		if (expectReplies == 0) timeToSleep = TIMEVAL_SUBTRACT(stime, now);
		else timeToSleep = 0;

		if (!lastNumProbesLeft || lastNumProbesLeft > numProbesLeft) {
			lastNumProbesLeft = numProbesLeft;
			snprintf(message, sizeof(message), 
				"\t\tTCP, UDP, ICMP Tests... %d probes left.", 
				numProbesLeft);
			PutNmapGlobalMessage(message);
		}
	} while (numProbesLeft > 0);

	PutNmapGlobalMessage("\tTCP, UDP, ICMP Tests done.");
}

static const char* FPString(HostOsScanInfo *hsi);
...
static void endRound(OsScanInfo *OSI, HostOsScan *HOS, int roundNum) 
{
	...
	char massage[1024];
	snprintf(massage, sizeof(massage),
		"Start TCP, UDP, ICMP Tests for %s. Count hosts %d, round %d.",
		o.takefingerprint ? "take fingerprints" : "OS detecting", 
		(int)OSI->incompleteHosts.size(), roundNum);
	PutNmapGlobalMessage(massage);

	for (hostI = OSI->incompleteHosts.begin(); hostI != OSI->incompleteHosts.end(); hostI++) {
		distance = -1;
		hsi = *hostI;
		// Build OS fingerprint for host.
		HOS->makeFP(hsi->hss);
		...
		//-------------------------------------------------------------------------
		// Next several rows inserted by Walter Artemenko, September 2019
		// if only the need is pointed to take OS fingerprints 
		// then made OS fingerprint without OS determinate 
		if (o.takefingerprint) {
			// Host is completed
			hsi->isCompleted = true;
			(*hostI)->FPs[roundNum] = hsi->FPs[roundNum];
			(*hostI)->FPR->FPs[roundNum] = hsi->FPR->FPs[roundNum];
			(*hostI)->FPR->numFPs = hsi->FPR->numFPs;
			if (o.verbose > 0) {
				printf("%s: %s: \n%s\n",
					hsi->target->targetipstr(),
					(hsi->target->hostname && strlen(hsi->target->hostname)) ? hsi->target->hostname : "_",
					FPString(hsi));
			}
			// Open file for write fingerprint.
			if (!o.fileFPs) {
				o.fileFPs = fopen(o.falenameFPs, "w");
				if (!o.fileFPs) {
					// Can't open file for writing host's fingerprints. 
					fatal("%s: Failed to open file for writing host's fingerprints.", __func__);
				}
			}
			// Documantatå results of OS fingerprint detect.
			std::string valueFP = FPString(hsi);
			fprintf(o.fileFPs, "%s: %s: %s\n",
				hsi->target->targetipstr(),
				(hsi->target->hostname && strlen(hsi->target->hostname)) ? hsi->target->hostname : "_",
				valueFP.c_str());

			snprintf(massage, sizeof(massage),
				"\t\tFingerprint received for %s: %s\n",
				hsi->target->targetipstr(), 
				valueFP.c_str());
			PutNmapGlobalMessage(massage);

			if (hsi->target->hostname && strlen(hsi->target->hostname)) {
				snprintf(massage, sizeof(massage),
					"\t\tFound domain name for    %s: %s\n",
					hsi->target->targetipstr(),
					hsi->target->hostname);
				PutNmapGlobalMessage(massage);
			}
			// Skipping OS determination
			continue;
		}
		//-------------------------------------------------------------------------
		// Further as in  orignal source code
...
//-------------------------------------------------------------------------
// Changed by Walter Artemenko, September 2019
static const char* FPString(HostOsScanInfo *hsi)
{
	FingerPrintResultsIPv4 *FPR = hsi->FPR;
	return mergeFPs(FPR->FPs, FPR->numFPs, true,
...
static void printFP(OsScanInfo *OSI) 
{
	...
	for (hostI = OSI->incompleteHosts.begin(); 
		...
		log_write(LOG_NORMAL | LOG_SKID_NOXLT | LOG_STDOUT,
			...
			hsi->target->targetipstr(), FPString(hsi));
	}
}
...
HostOsScanInfo::HostOsScanInfo(Target *t, OsScanInfo *OsSI) {
  ...
  // Changed by Walter Artemenko, Septembe 2019
  target->osscanSetFlag(!o.takefingerprint ? OS_PERF : OS_NOTPERF);
  ...
  for (targetno = 0; targetno < Targets.size(); targetno++) {
    ...
    if (Targets[targetno]->ports.getStateCounts(IPPROTO_TCP, PORT_OPEN) == 0 ||
      ...
      if (o.osscan_limit) {
		...
		//---------------------------------------------------------------------
		// Changed by Walter Artemenko, Septembe 2019.
		Targets[targetno]->osscanSetFlag(
			!o.takefingerprint ? OS_PERF_UNREL : OS_NOTPERF);
		//---------------------------------------------------------------------
      }
    }
...
int OSScan::os_scan_ipv4(std::vector<Target *> &Targets) 
{
	...
	if (!Targets.size()) return OP_FAILURE;

	perf.init();

	OsScanInfo OSI(Targets);
	// no one will be scanned
	if (!OSI.numIncompleteHosts()) return OP_FAILURE;
	...
	snprintf(message, sizeof(message), "%s for %d targets",
		o.takefingerprint ? "Fingerprint started" : "OS detecting started",
		(int)Targets.size());
	PutNmapGlobalMessage(message);

	while (OSI.numIncompleteHosts() != 0) {
		if (itry >  0)	sleep(1);
		// Try waiting a little longer just in case it matters
		if (itry == 3)	usleep(1500000);  
		if (o.verbose) {
			counthosts = OSI.numIncompleteHosts();
			char targetstr[128];
			if (counthosts == 1)
				(*(OSI.incompleteHosts.begin()))->target->NameIP(targetstr, sizeof(targetstr));
			else 
				Snprintf(targetstr, sizeof(targetstr), "%d hosts", counthosts);
	...
	counthosts = OSI.numIncompleteHosts();
	if (counthosts) {
		// For hosts that don't have a perfect match, find the closest 
		// fingerprint in the DB and, if we are in debugging mode, print them. 
		findBestFPs(&OSI);
		snprintf(message, sizeof(message), "%s%d%s",
		"For ", counthosts, 
		" hosts that don't have a perfect match, find the closest fingerprint in the DB. Done.");
		PutNmapGlobalMessage(message);
		if (o.debugging > 1) printFP(&OSI);
	}

	snprintf(message, sizeof(message), "%s done.",
		o.takefingerprint ? "Fingerprint" : "OS detecting");
	PutNmapGlobalMessage(message);
...
int OSScan::os_scan_ipv6(std::vector<Target *> &Targets) {
	...
	// Safe checks. 
	if (Targets.size() == 0) return OP_FAILURE;
...
int OSScan::os_scan(std::vector<Target *> &Targets) 
{
	...
	for (size_t i = 0; i < Targets.size(); i++) {
		((Targets[i]->af()==AF_INET6) ? ip6_targets : ip4_targets).
			push_back(Targets[i]);
	}

	// Do IPv4 OS Detection
	if (ip4_targets.size() > 0)	res4 = os_scan_ipv4(ip4_targets);
	// Do IPv6 OS Detection 
	if (ip6_targets.size() > 0)	res6 = os_scan_ipv6(ip6_targets);

	// If both scans were successful, return OK 
	return (res4==OP_SUCCESS && res6==OP_SUCCESS) ? OP_SUCCESS : OP_FAILURE;
}

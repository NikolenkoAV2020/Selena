...
void parse_options(int argc, char **argv) {
  ...
  struct option long_options[] = {
	...
	{"take-finger-print", no_argument, 0, 0 },
    {0, 0, 0, 0}
  };
  ...
  while ((arg = getopt_long_only(argc, argv, "46Ab:D:d::e:Ffg:hIi:M:m:nO::o:P::p:qRrS:s::T:Vv::", long_options, &option_index)) != EOF) {
    ...
    case 0:
      ...
      if (strcmp(long_options[option_index].name, "script") == 0) {
		...
		} else if (strcmp(long_options[option_index].name, "take-finger-print") == 0) {
			o.takefingerprint = true;
			o.osscan = false;
		} else {
    ...
    case 'O':
	  ...
	  //-----------------------------------------------------------------------
	  // Inserted Walter Artemenko, September 2019.
	  // take-finger-print
	  else if (!strcmp(optarg, "tfp")) {
		  // Need only take finger prints for every host 
		  // without OS detection. 
		  // Host fingerprint comparison with BD of FP is forbidden.
		  // Prints received are documented later to output file or/and to screan.
		  o.takefingerprint = true;
		  o.osscan = false;
	  }
	  //-----------------------------------------------------------------------
...
void  apply_delayed_options() {
  ...
  // In orignal source code was "if (o.osscan) {".
  // If o.takefingerprint option is on it mean that 
  // need the only take fingerprints wishout OS detection and 
  // nmap-os-db file not use, comparison fingerprints not performed.s 
  if (o.osscan && !o.takefingerprint) {
    if (o.af() == AF_INET)
  ...
  if (delayed_options.exclude_file) {
	...
	delayed_options.exclude_file = NULL;
  }

  o.exclude_spec = delayed_options.exclude_spec;
  delayed_options.exclude_spec = NULL;
...
int nmap_main(int argc, char *argv[]) {
  ...
  // Added by Walter Artemenko, September 2019
  if (o.exclude_group) {
	  free(o.exclude_group);
	  o.exclude_group = NULL;
  }
  o.exclude_group = addrset_new();

  /* lets load our exclude list */
  if (o.excludefd != NULL) {
    load_exclude_file(o.exclude_group, o.excludefd);
    fclose(o.excludefd);
	o.excludefd = NULL;
  }
  if (o.exclude_spec != NULL) {
    load_exclude_string(o.exclude_group, o.exclude_spec);
  }

  if (o.debugging > 3)
    dumpExclude(o.exclude_group);
  ...
  do {
	...
	while (Targets.size() < ideal_scan_group_sz) {
		...
		currenths = nexthost(&hstate, o.exclude_group, &ports, o.pingtype);
		if (!currenths)
			break;
		...
		if ((o.noportscan && !o.traceroute
			...
			delete currenths;
			o.numhosts_scanned++;
			if (!o.max_ips_to_scan || 
				o.max_ips_to_scan > o.numhosts_scanned + Targets.size())
				continue;
			else
				break;
		}
	...
	//-------------------------------------------------------------------------
	// Inserted by Walter Artemenko, September 2019
	snprintf(message, sizeof(message), "Build %d targets & dns-revers done.", (int)Targets.size());
	PutNmapGlobalMessage(message);
	//-------------------------------------------------------------------------

	// Couldn't find any more targets
    ...
    if (!o.noportscan) {
		...
		if (o.idlescan) {
			snprintf(message, sizeof(message), "Start Idle scaning for %d targets.", (int)Targets.size());
			PutNmapGlobalMessage(message);

			for (targetno = 0; targetno < Targets.size(); targetno++) {
				o.current_scantype = IDLE_SCAN;
				// Check if a status message should be printed
				keyWasPressed(); 
				idle_scan(Targets[targetno], ports.tcp_ports,
					ports.tcp_count, o.idleProxy, &ports);
			}
			PutNmapGlobalMessage("\tIdle scaning done.");
		}
		if (o.bouncescan) {
			snprintf(message, sizeof(message), "Start Bounce scaning for %d targets.", (int)Targets.size());
			PutNmapGlobalMessage(message);
			...
			PutNmapGlobalMessage("\tBounce scaning done.");
		}
	...
	if (o.osscan || o.takefingerprint) {
		...
		if (o.osscan) PutNmapGlobalMessage("\tOS info scaning done.");
		else PutNmapGlobalMessage("\tOS fingerprints scaning done.");
	}
  ...
  if (o.script) {
	  ...
	  if (new_targets) delete new_targets;
	  new_targets = NULL;
  }
#endif
  ...
  // Inserted by Valter Artyomenko august 2019
  nmap_main_free();
...
// Inserted by Valter Artyomenko august 2019
// Free all captured resources befor.
void nmap_main_free()
{
	addrset_free(o.exclude_group);
	o.exclude_group = NULL;

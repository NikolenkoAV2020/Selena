...
static void fatal(char *fmt, ...) 
{
	...
	va_start(ap, fmt);
	fflush(stdout);
	snprintf(msg, 1024, fmt, ap);
	va_end(ap);

	// Code was changed by Walter Artyomenko, august 2019
...
{
	void *mymem = NULL;
	if ((int)size <= 0) fatal("Tried to malloc negative amount of memory!!!");
	else {
		mymem = malloc(size);
		if (!mymem) fatal("Malloc Failed! Probably out of space.");
	}
	return mymem;
}
...
{
	void *mymem = NULL;
	if ((int)size <= 0) fatal("Tried to realloc negative amount of memory!!!");
	else {
		mymem = realloc(ptr, size);
		if (!mymem) fatal("Realloc Failed! Probably out of space.");
	}
	return mymem;
}

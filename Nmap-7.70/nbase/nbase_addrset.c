...
void addrset_free(struct addrset *set)
{
	...
	if (set->trie) trie_free(set->trie);
    free(set);
}
...
static char *address_to_string(const struct sockaddr *sa, size_t sa_len,
                               char *buf, size_t len)
{
    getnameinfo(sa, (socklen_t)sa_len, buf, (socklen_t)len, NULL, 0, NI_NUMERICHOST);
    return buf;
}
...
/* Add whitespace-separated host specifications from fd into the address set.
   Returns 1 on success, 0 on error, -999 fatal error */
int addrset_add_file(struct addrset *set, FILE *fd, int af, int dns)
{
    ...
    for (;;) {
		...
		int r = addrset_add_spec(set, buf, af, dns);
		if (!r) return 0;
		if (r == -999) return -999;
    }

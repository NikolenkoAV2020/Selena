...
const char *inet_ntop_ez(const struct sockaddr_storage *ss, size_t sslen) {
  ...
  if (sin->sin_family == AF_INET) {
    if (sslen < sizeof(struct sockaddr_in))
      return NULL;
    return inet_ntop(AF_INET, (void*)&sin->sin_addr, (char*)str, (size_t)sizeof(str));
  }
#if HAVE_IPV6
  else if(sin->sin_family == AF_INET6) {
    if (sslen < sizeof(struct sockaddr_in6))
      return NULL;
    return inet_ntop(AF_INET6, (void*)&sin6->sin6_addr, (char*)str, sizeof(str));
  }
#endif
...
const char *sockaddr_storage_iptop(const struct sockaddr_storage * addr, char * dst)
{
  ...
  case AF_INET:
  {
    ...
    return inet_ntop(addr->ss_family, (void*)&(ipv4_ptr->sin_addr), dst, INET_ADDRSTRLEN);
  }
#if HAVE_IPV6
  case AF_INET6:
  {
    const struct sockaddr_in6 * addrv6p = (struct sockaddr_in6 *) addr;
    return inet_ntop(addr->ss_family, (void*)&(addrv6p->sin6_addr), dst, INET6_ADDRSTRLEN);
  }
#endif
  default:
  {

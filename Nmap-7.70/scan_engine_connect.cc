...
bool do_one_select_round(UltraScanInfo *USI, struct timeval *stime) 
{
	...
	while ((incompleteHostI != USI->incompleteHosts.end()
		...
		// Original code wrote by author of nmap
		/*
		...
		*/
		...
		// Code corrected by Walter Artyomenko
		int listSize = host->probes_outstanding.size();
		int listCount = 0;
		std::list<UltraProbe *>::iterator probeI = host->probes_outstanding.begin();
		...
		while (listSize  > 0 &&
			 ...
			 * the iterator. We copy and increment it here instead of in the for-loop
			 * statement to avoid incrementing an invalid iterator */
			std::list<UltraProbe *>::iterator nextProbeI = probeI;
			nextProbeI++;
			...
			listSize = host->probes_outstanding.size();

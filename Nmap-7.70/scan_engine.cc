	...
	struct timeout_info *to) 
{
...
#ifdef WIN32
	if (g_has_npcap_loopback == 0 && scantype != CONNECT_SCAN && 
		Targets[0]->ifType() == devt_loopback) {
		log_write(LOG_STDOUT, 
			"Skipping %s against %s because Windows does not support scanning your own machine (localhost) this way.\n", 
	...
	switch (scantype) {
	case STYPE_UNKNOWN:		name = "Unknown"; break;
	case HOST_DISCOVERY:	name = "Host discovery"; break;
	case ACK_SCAN:			name = "Ack"; break;
	case SYN_SCAN:			name = "Syn"; break;
	case FIN_SCAN:			name = "Fin"; break;
	case XMAS_SCAN:			name = "Xmas"; break;
	case UDP_SCAN:			name = "Udp"; break;
	case CONNECT_SCAN:		name = "Connect"; break;
	case NULL_SCAN:			name = "Null"; break;
	case WINDOW_SCAN:		name = "Window"; break;
	case SCTP_INIT_SCAN:	name = "Sctp init"; break;
	case SCTP_COOKIE_ECHO_SCAN: name = "Sctp cookie"; break;
	case MAIMON_SCAN:		name = "Maimon"; break;
	case IPPROT_SCAN:		name = "Ip prot"; break;
	case PING_SCAN:			name = "Ping"; break;
	case PING_SCAN_ARP:		name = "Ping arp"; break;
	case IDLE_SCAN:			name = "Idle"; break;
	case BOUNCE_SCAN:		name = "Bounce"; break;
	case SERVICE_SCAN:		name = "Service"; break;
	case OS_SCAN:			name = "OS"; break;
	case SCRIPT_PRE_SCAN:	name = "Pre script"; break;
	case SCRIPT_SCAN:		name = "Script"; break;
	case SCRIPT_POST_SCAN:	name = "Post script"; break;
	case TRACEROUTE:		name = "Trace route"; break;
	case PING_SCAN_ND:		name = "Ping ND"; break;
	}
	char message[1024];
	snprintf(message, sizeof(message), "Start %s scaning for %d targets.", 
		name, (int)Targets.size());
	...
	snprintf(message, sizeof(message), "\t%s scaning done.", name);
	PutNmapGlobalMessage(message);
}

...
class NmapOps {
 public:
  ...
  void RecourceFree();	// Free all early allocated resources...
...
#ifdef _LIB
  //---------------------------------------------------------------------------
  // For redirect output - inserted by Walter Artemenko, September 2019
  std::string JobName;				// Name of job for create output file name.
  std::string DirName;				// Directory name for results.
#endif

  //---------------------------------------------------------------------------
  // For fingerprints - inserted by Walter Artemenko, September 2019
  bool  takefingerprint;			// take finger print for every targets
  FILE* fileFPs;					// fale for save fingerprints
  char  falenameFPs[512];			// File name for save fingerprints

...
//-----------------------------------------------------------------------------
// Insert Walter Artemenko, September 2019
#include <SelenaSafeObject.h>
#include <iostream>

class NmapGlobalState {
public :
	...
	void GetGlobalState(bool& isFatal, bool& isMsg, bool& isWar) {
		isFatal = IsFatal();
		isMsg   = isNewMessage;
		isWar   = isNewWarning;
	}
	bool IsFatal() { return FatalError.length() ? true : false; }

	void PutMsg(const char*  msg) {
		Messages.push_back(msg);
		isNewMessage = true;
	}
	void PutWar(const char*  war) {
		Warnings.push_back(war);
		isNewWarning = true;
	}
	void PutFat(const char*  fat) {
		FatalError = fat;
	}
	...
	void GetWar(std::string& war) {
		...
		war = Warnings.front();
		Warnings.pop_front();
		if (!Warnings.size()) isNewWarning = false;
	}
	void GetFat(std::string& fat) {
		fat = FatalError;
		fat = "";
	}
...
typedef ZSelenaSafeObject<NmapGlobalState>	SafeGlobalState;
SafeGlobalState GlobalState;

#ifdef __cplusplus
extern "C" {
#endif // #ifdef __cplusplus

	void InitNmapGlobalState() {
		GlobalState->V().Init();
	}

	void PutNmapGlobalMessage(const char* msg) {
#ifdef _LIB
		//GlobalState->V().PutMsg(msg);
#else
		std::cout << msg << "\n";
		fflush(stdout);
#endif
	...
	void PutNmapGlobalWarning(const char* war) {
#ifdef _LIB
		//GlobalState->V().PutWar(war);
#else
		std::cout << "Warning: " << war << "\n";
		fflush(stdout);
#endif
	}

	void PutNmapGlobalFatal(const char* fat) {
#ifdef _LIB
		//GlobalState->V().PutFat(fat);
#else
		std::cout << "Fatal error: " << fat << "\n";
		fflush(stdout);
#endif
	}

	void GetNmapGlobalMessage(std::string& msg) {
		GlobalState->V().GetMsg(msg);
	}

	void GetNmapGlobalWarning(std::string& war) {
		GlobalState->V().GetWar(war);
	}

	void GetNmapGlobalFatal(std::string& fat) {
		GlobalState->V().GetFat(fat);
	}

	void GetNmapGlobalState(bool& isFatal, bool& isMsg, bool& isWar) {
		GlobalState->V().GetGlobalState(isFatal, isMsg, isWar);
	}
...
//-----------------------------------------------------------------------------
// If Nmap buid as a static library
#ifdef _LIB

// Include common Selena soft header files
#include "./../SourceCommon/DecodeCommandString.h"
#include "./../SourceCommon/LibNmapScan.h"
#ifdef _WIN32
	#include "./libnetutil/netutil.h"
#else
	#include "./../../libnetutilCent/libnetutil/netutil.h"
#endif

int newNMapMain(int argc, char** argv);

// Execut Nmap as function with users command "Command"
int LaunchNmap(
	...
	const char* DirName)	// Directory name for results.
{
	...
	o.JobName = JobName;
	o.DirName = DirName;
	o.InitializeFingerPrint();
	InitNmapGlobalState();

	int r = newNMapMain(countString, strings);
...
// New Nmap main function to replace old native main procedure
// of executable file.
int newNMapMain(int argc_, char** argv_)

#else

//-----------------------------------------------------------------------------
// If Nmap buid as a executabl file
...
#endif // #ifdef _LIB
{
  ...
  */
  ...
  //---------------------------------------------------------------------------
  // Inserted by Walter Artemrnke, September 2019,
  // for interception exit() function that was replaced on throw()
  try {
  //---------------------------------------------------------------------------
	  ...
	  if (cptr) {
		  ...
		  for (int i = 1; i < argc && strlen(command) + strlen(argv[i]) + 1 < sizeof(command); i++) {
			  strcat(command, " ");
			  strcat(command, argv[i]);
		  }
		  char**myargv = NULL;
		  int   myargc = arg_parse(command, &myargv);
		  if (myargc < 1) {
			  fatal("NMAP_ARGS variable could not be parsed");
		  }
		  int ret = nmap_main(myargc, myargv);
  ...
  // for interception exit() function that was replaced on throw()
	  ...
	  // Show happened fatal error or warning

	  // free all allocated in nmap_main function rosources befor
	  nmap_main_free();
	  return 1;
  }
  //---------------------------------------------------------------------------

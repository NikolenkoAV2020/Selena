...
NmapOps::~NmapOps() {
	// Changed by Valter Artyomenko august 2019
	RecourceFree();
}

// Inserted by Valter Artyomenko august 2019
void NmapOps::RecourceFree() {
	unsigned int n = os_labels_ipv6.size();
	if (n > 0) {
		for (int i = 0; i<(int)n; ++i) {
			os_labels_ipv6[i].OS_class.clear();
		}
		os_labels_ipv6.clear();
	}

	requested_data_files.clear();
	loaded_data_files.clear();
...
//-----------------------------------------------------------------------------
// takefingerprint - inserted by Walter Artemenko, September 2019
void NmapOps::InitializeFingerPrint() 
{
	takefingerprint = false;
	fileFPs = NULL;
	falenameFPs[0] = '\0';
#ifdef _LIB
	if (o.DirName.length() && o.JobName.length()) {
		snprintf(falenameFPs, 512, "%s/%sfingerprint.txt",
			o.DirName.c_str(), o.JobName.c_str());
	} else {
		snprintf(falenameFPs, 512, "nmapHostFingerPrint.txt");
	}
#else
	snprintf(falenameFPs, 512, "nmapHostFingerPrint.txt");
#endif
